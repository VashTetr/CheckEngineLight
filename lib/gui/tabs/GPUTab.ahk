class GPUTab {
    ...
    Create() {
        ...
        this.controls["title"] := g.Add("Text", "x" x " y" y " w" w " h30 Hidden c" ThemeManager.TextPrimary " Background" ThemeManager.BgContent, "GPU Drivers")
    ...
    Update(results) {
        ...
        if (driverCount > 0)
            this.controls["summary"].Text := driverCount " GPU driver(s) — " status ": " message
        else
            this.controls["summary"].Text := "GPU: " message
    ...
    SetDriverDetails(allDrivers, maxAgeMonths) {
        ...
        Loop this.maxDrivers {
            ...
            if (i <= allDrivers.Length) {
                ...
                row["label"].Text := drv["name"] "  v" drv["version"] "  (" dateStr ", " Round(age) " months)  [" drv["provider"] "]"
